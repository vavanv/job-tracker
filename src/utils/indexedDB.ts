...
export const exportApplicationsAsJSON = async (): Promise<string> => {
  ...
  const exportData = await Promise.all(applications.map(async (app) => ({
    ...
    resumeBlob: app.resumeBlob ? await blobToBase64(app.resumeBlob) : undefined,
    coverLetterBlob: app.coverLetterBlob ? await blobToBase64(app.coverLetterBlob) : undefined,
...
export const importApplicationsFromJSON = async (jsonData: string): Promise<{ imported: number; errors: string[] }> => {
  try {
    ...
    if (data.applications && Array.isArray(data.applications)) {
      ...
      applicationsData = data.jobs.map((job: any) => ({
        ...
        companyWebsite: job.company?.includes('http') ? job.company.replace(/`/g, '').trim() : undefined,
        jobTitle: job.position || '',
        applicationDate: job.dateApplied || job.createdAt,
        status: job.status || 'Applied',
        notes: [job.notes, job.salaryRange, job.jobType, job.location].filter(Boolean).join('\n'),
        jobLink: job.url?.replace(/`/g, '').trim() || undefined,
        createdAt: job.createdAt,
        updatedAt: job.updatedAt,
        // Handle resume data if present (will be processed later)
         resumeData: job.resume?.data && job.resume.data.trim() ? job.resume.data : undefined,
         resumeMeta: job.resume && job.resume.data && job.resume.data.trim() ? {
         ...
         coverLetterData: job.coverLetter?.data && job.coverLetter.data.trim() ? job.coverLetter.data : undefined,
         coverLetterMeta: job.coverLetter && job.coverLetter.data && job.coverLetter.data.trim() ? {
           fileName: job.coverLetter.name || 'cover-letter.docx',
           mimeType: job.coverLetter.type || 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
           size: job.coverLetter.size || 0,
           updatedAt: job.updatedAt || job.createdAt
         } : undefined
      }));
    } else {
      throw new Error("Invalid JSON format: missing applications or jobs array");
    }
    ...
    for (const appData of applicationsData) {
      try {
        ...
        if (!appData.companyName || !appData.jobTitle || !appData.applicationDate) {
          errors.push(`Skipping application: missing required fields (companyName, jobTitle, or applicationDate)`);
          continue;
        }
        ...
        const application: Application = {
          ...
          resumeBlob: appData.resumeBlob && typeof appData.resumeBlob === 'string' 
            ? await base64ToBlob(appData.resumeBlob) 
            : appData.resumeData 
            ? await base64ToBlob(appData.resumeData, appData.resumeMeta?.mimeType)
            : undefined,
          coverLetterBlob: appData.coverLetterBlob && typeof appData.coverLetterBlob === 'string' 
            ? await base64ToBlob(appData.coverLetterBlob) 
            : appData.coverLetterData
            ? await base64ToBlob(appData.coverLetterData, appData.coverLetterMeta?.mimeType)
            : undefined,
        ...
        errors.push(`Failed to import application "${appData.companyName} - ${appData.jobTitle}": ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
...
const base64ToBlob = async (base64: string, mimeType: string = 'application/octet-stream'): Promise<Blob> => {
